...
import com.example.coupleapp.Adapter.AlbumViewAdapter;
...
public class AlbumViewActivity extends AppCompatActivity {
    ...
    private ArrayList<AlbumViewData> mArrayList;             // 모델 클래스의 데이터를 받아 리사이클러뷰에 뿌리는 데 쓸 ArrayList
    private AlbumViewAdapter mAdapter;                       // 리사이클러뷰 어댑터
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_title.setText(album);
        imageButton_back = findViewById(R.id.imageButton_back);

        imageButton_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(AlbumViewActivity.this,StoryActivity.class);
        ...
        // 리사이클러뷰 선언
        rv = (RecyclerView) findViewById(R.id.recyclerview_albumview);
        // 프래그먼트기 때문에 context가 아니라 getActivity()를 쓴다!!!
        rv.setLayoutManager(new GridLayoutManager(AlbumViewActivity.this,3));

        // 리사이클러뷰에 연결되서 데이터를 뿌릴 ArrayList 선언
        mArrayList = new ArrayList<>();

        // 프래그먼트에서 리사이클러뷰의 어댑터를 붙일 땐 context 쓰는 부분에 getActivity()를 쓴다
        mAdapter = new AlbumViewAdapter(mArrayList,AlbumViewActivity.this);
