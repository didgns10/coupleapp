...
public class DateReviewUpdateActivity extends AppCompatActivity {
    ...
    private String contentid,contenttypeid,review_idx;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_date_review_update);

        //인텐트 받는거
        contentid = getIntent().getStringExtra("contentid");
        contenttypeid = getIntent().getStringExtra("contenttypeid");
        rate = getIntent().getStringExtra("rate");
        content = getIntent().getStringExtra("review");
        review_img = getIntent().getStringExtra("review_img");
        review_idx = getIntent().getStringExtra("review_idx");

        Log.e("리뷰수정",contentid);
        Log.e("리뷰수정",contenttypeid);
        Log.e("리뷰수정",rate);
        Log.e("리뷰수정",content);
        Log.e("리뷰수정",review_img);
        Log.e("리뷰수정",review_idx);

        rates = Float.parseFloat(rate);
        ...
        et_story_title.setText(content);
        ratingBar2.setRating(rates);
        Glide.with(this).load(review_img).into(imgv_review);
    ...
    private void uploadImage(){
        ...
        Call<ReviewClass> call = apiInterfaceReview.updateReview(name,imgurl,date,rate,content,review_img,contentid,contenttypeid,review_idx);
        ...
        call.enqueue(new Callback<ReviewClass>() {
            ...
            public void onResponse(Call<ReviewClass> call, Response<ReviewClass> response) {
                ...
                if(reviewClass.getResponse().equals("no")){
                    Toast.makeText(DateReviewUpdateActivity.this,"빈칸없이 채워주세요",Toast.LENGTH_SHORT).show();
                }else if(reviewClass.getResponse().equals("yes")){

                    Toast.makeText(DateReviewUpdateActivity.this,"리뷰수정을 완료했습니다.",Toast.LENGTH_SHORT).show();
                    finish();

                }else{
                    Toast.makeText(DateReviewUpdateActivity.this,"리뷰작성에 실패하셨습니다.",Toast.LENGTH_SHORT).show();
                }
